...
# ---------------------------------------------------------------------------
# Timezone helpers – keep storage naive, convert to aware when needed
# ---------------------------------------------------------------------------
def get_utc_now_naive() -> datetime:
    """Return a naive UTC datetime (for storing in SQLite)."""
    return datetime.now(timezone.utc).replace(tzinfo=None)


def make_aware(dt: Optional[datetime]) -> Optional[datetime]:
    """Add UTC timezone to a naive datetime (for comparisons)."""
...
# ---------------------------------------------------------------------------
# Status Constants – single source of truth for all state names
# ---------------------------------------------------------------------------
class LotStatus:
    OPEN = "Open"
    MATCHED = "Matched"
...
class User(SQLModel, table=True):
    ...
    email: Optional[str] = Field(default=None, unique=True, index=True)  # <-- unique email
...
class PasswordResetToken(SQLModel, table=True):
    ...
    # --- Brute-force protection & resend limits for password reset ---
    attempts: int = Field(default=0)              # failed reset code attempts
    last_attempt_at: Optional[datetime] = Field(default=None)
    resend_count: int = Field(default=0)          # number of times reset code resent
    last_resend_at: Optional[datetime] = Field(default=None)
...
class Match(SQLModel, table=True):
    ...
    # --- Deadlines (enforced by background watchdog) ---
